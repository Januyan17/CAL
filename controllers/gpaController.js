...
async function createGPA(req, res, next) {
  ...
  if (req.body.type === 'GPA') {
    ...
    try {
      ...
      if (skill) {
        knowledgeSkill = knowledgeSkill + skill?.knowledge;
        psSkill = psSkill + skill?.problem_solving;
        creativity = creativity + skill?.creativity;
    ...
    const { club_name, club_level } = req.body;
    const payload = {
      club_name,
      club_level,
      file_id: file ? file.id : null,
    ...
    const { sport_name, level, cardinality, position } = req.body;
    const payload = {
      sport_name,
      level,
      cardinality,
    ...
    const { event_name, level, cardinality, position } = req.body;
    const payload = {
      event_name,
      level,
      cardinality,
